...
//Variable declaration
    ...
    public static int s, x, count = 0, c1, c2, card1, card2, temp, left = 16, striker = 51, looper = 0;
    ...
    private void p1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_p1ActionPerformed
        if (count < 2) {  //Only execute if count is <2 so no 3rd click
            looper++;
            if (looper == 1) {      //start timer
                timings();
            }
            temp = t[0];
            iconsetter(temp);     //set icon
            ...
            p1.setEnabled(false);   //disable so no double click
            countcheck();
...
//Check button
        if (count == 2) {
            ...
            if (left == 0) {    //Checking if game done or not if yes display congratulation
                try {
                    ti.stop();
    ...
    public ImageIcon iconsetter(int temp) {
        switch (temp) {      //Setting icon to button
            case 0:
                ic = a;
                break;
            case 1:
    ...
    public void countcheck() {
        if (count == 0 || count == 1) {       //Enable and disable check button
            check.setEnabled(false);
        } else {
    ...
    public void timings() {      //Timer display
        ...
        ti = new Timer(1000, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                striker--;
                timeprint.setText("Time Left: " + striker);
    ...
    public void assignment() {
        ...
        for (int y = 0; y < 2; y++) {
            for (x = 0; x < 8; x++) {    //ArrayList
                nums.add(x);
            }
        }
        int dab;
        for (dab = 0; dab <= 20; dab++) {
            change[dab] = dab + 4;      //Change array
        }

        Collections.shuffle(nums);

        for (x = 0; x <= 15; x++) {
            t[x] = (int) nums.get(x);    // Assignment
        }
